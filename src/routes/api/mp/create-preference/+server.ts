...
import { createOrder } from "$lib/server/order.repo";
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  if (!sellerId || !amount || !title || !external_reference) {
    throw error(
      400,
      "Missing required parameters: sellerId, amount, title, external_reference"
    );
  }
  ...
  if (!seller || !seller.access_token) {
    throw error(
      404,
      `Seller ${sellerId} not found or not connected to Mercado Pago`
    );
  }
  ...
  try {
    ...
    console.log(`📝 Orden creada en DB: ${order.id} (${external_reference})`);
    ...
    const preference = await new Preference(mpClient).create({
      body: {
        ...
        // Auto-retorno solo para approved y pending (rejected permite retry en MP)
        auto_return: "approved",
        // Webhook para confirmación server-side
        notification_url:
          PUBLIC_WEBHOOK_URL || `${PUBLIC_APP_URL}/api/mp/webhook`,
      },
    });
